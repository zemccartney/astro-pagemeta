...
let nextDefaultPort = 10_000 + Math.floor(Math.random() * 40_000);
...
export async function loadFixture({
    ...
    return {
        ...
        fetch: async (url, init) => {
            ...
            try {
                const res = await request(resolvedUrl, init);
        ...
        loadTestAdapterApp: async () => {
            ...
            debug(
                `Importing test adapter entrypoint from ${entryUrl.toString()}`
            );
            const mod = (await import(
                /* @vite-ignore */ entryUrl.toString()
            )) as {
            ...
            return {
                render: (req: Request) => app.render(req),
                toInternalApp: () => app
            };
        },
