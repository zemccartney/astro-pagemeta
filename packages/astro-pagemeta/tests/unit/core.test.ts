...
describe("Content-Security-Policy rejection", () => {
    const processor = createMetadataProcessor({
        addRequiredGlobalMeta: false,
        compressHTML: false,
        routePatterns: []
    ...
    test("throws when page metadata sets a CSP custom key", () => {
        const ctx = mockContext();
        metadata(ctx, {
            custom: { "content-security-policy": "default-src 'self'" }
        });
        expect(() => processor.resolveMetadata(ctx)).toThrow(
            /Content-Security-Policy cannot be set via pagemeta/
        );
    });

    test("throws case-insensitively", () => {
        const ctx = mockContext();
        metadata(ctx, {
            custom: { "Content-Security-Policy": "default-src 'self'" }
        });
        expect(() => processor.resolveMetadata(ctx)).toThrow(
            /use Astro's built-in CSP support/i
        );
    });

    test("throws when object defaults set a CSP custom key", () => {
        const withDefaults = createMetadataProcessor({
            addRequiredGlobalMeta: false,
            compressHTML: false,
            defaults: {
                custom: { "CONTENT-SECURITY-POLICY": "default-src 'self'" }
        ...
        expect(() => withDefaults.resolveMetadata(mockContext())).toThrow(
            /Content-Security-Policy cannot be set via pagemeta/
        );
    });

    test("throws when function defaults return a CSP custom key", () => {
        const withDefaults = createMetadataProcessor({
            addRequiredGlobalMeta: false,
            compressHTML: false,
            defaults: () => ({
    ...
    test("non-CSP custom keys pass through untouched", () => {
        const ctx = mockContext();
        metadata(ctx, { custom: { "csp-report-note": "not the real header" } });
        expect(processor.resolveMetadata(ctx)).toEqual({
            custom: { "csp-report-note": "not the real header" }
        });
    });
});
