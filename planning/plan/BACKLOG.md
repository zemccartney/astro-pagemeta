...
- **Astro 7.3 `astro/middleware` invalidation cone: not filing upstream** (Zack, 2026-09-20). Local bug report with the chain, the verified trace, the current repro and a proposed standalone one: [`planning/artifacts/astro-7.3-middleware-invalidation.md`](../artifacts/astro-7.3-middleware-invalidation.md). Zack to reproduce more simply and revisit as time allows.
- ~~**Drop the runtime import of `astro/middleware`.**~~ _Declined 2026-09-20: `Symbol.for` stays as the fix; a change that only works because we inferred something about Astro's internals is exactly the kind of "magical" fix Zack does not want. Revisit only if upstream documents the behavior._
