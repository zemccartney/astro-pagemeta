...
- [ ] TypeScript 6 — attempted 2026-07-04, **deferred until after the Astro 6 bump**: typecheck/lint/astro-check all pass under TS 6, but Astro 5's internals (`tsconfck`, `zod-to-ts`) peer on TS ^5 — don't carry a peer-violating compiler into the riskiest upgrade step. Retry immediately after the bump (Astro 6's tooling should declare TS 6 support).
- [ ] Bump catalog `astro` → `^6`; run full suite, fix fallout
