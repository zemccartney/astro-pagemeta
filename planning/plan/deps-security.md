...
_Update 2026-07-04 (found by Zack):_ ncu's **interactive mode is broken with `--workspaces`** — it prompts for the root package.json only, silently skipping the rest. Non-interactive workspace reports work fine (verified: covers root, all packages, and the catalog). Scripts adjusted accordingly: `pnpm deps` = non-interactive workspace-wide report (cooldown-gated); `pnpm deps:all` = same without cooldown (full picture). For interactive ad-hoc tidying, run ncu directly inside the package: `cd packages/astro-pagemeta && npx ncu -p pnpm -i --format group --cooldown 7d` (a root script can't do this — pnpm scripts don't run against a subdirectory's package.json).
